...
test('every countable step carries its count, and the tick follows the count', () => {
  ...
  assert.deepEqual(
    { have: stones.count.have, need: stones.count.need, done: stones.done },
  ...
  assert.deepEqual(
    { have: after.count.have, need: after.count.need, done: after.done },
    { have: 3, need: 3, done: true },
    'a count never reads more than it needs',
  );
...
test('never more than two jobs at once, and the rest are only waiting', () => {
  ...
  assert.deepEqual(
    shown.map(p => p.id),
    queue.slice(0, MAX_ACTIVE).map(p => p.id),
  ...
  assert.ok(
    activeProblems(w).every(p => p.id !== third),
    'the third job is not shown yet',
  );
  w.players.A.res.plank = 9;
  w.players.A.res.stone = 9;
  applyAction(w, { type: 'bridge.build', role: 'A', planks: 5, stone: 4, quality: 3 });
...
test('the children use the playground once it is there', () => {
  const w = settled(9);
  w.players.A.res.plank = PROJECT.play.plank;
  w.players.A.res.stone = PROJECT.play.stone;
  ...
  for (let i = 0; i < 3000 && !seen; i++) {
    tick(w);
    seen = little.some(
      v => v.x >= pg.x - 1 && v.x <= pg.x + pg.w + 1 && v.y >= pg.y - 1 && v.y <= pg.y + pg.h + 1,
    );
  }
...
test('a fenced field is one the sheep leave alone', () => {
  ...
  let strayed = false;
  for (let i = 0; i < 6000 && !strayed; i++) {
    tick(w);
    strayed = w.sheep.some(inField);
  }
