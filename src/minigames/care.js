...
export function openCare(game, sheep) {
  ...
  const slots = ITEMS.map((it, i) =>
    Object.assign({}, it, { hx: 60 + i * 100, hy: 258, x: 60 + i * 100, y: 258, held: false }),
  );
  let dragging = null,
    moved = 0,
    shake = 0,
  ...
  onPointer(cv.canvas, 420, 300, {
    down(pt) {
      ...
      for (const s of slots) {
        if (Math.abs(pt.x - s.x) < 36 && Math.abs(pt.y - s.y) < 36) {
          dragging = s;
          s.held = true;
          return;
        }
  ...
  function draw(t) {
    ...
    ctx.fillStyle = '#c8e0bd';
    ctx.fillRect(0, 190, 420, 110);
    ctx.strokeStyle = 'rgba(120,160,110,.5)';
    ctx.lineWidth = 2;
    ctx.lineCap = 'round';
    for (let i = 0; i < 22; i++) {
      const gx = (i * 37) % 420,
      ...
      ctx.beginPath();
      ctx.moveTo(gx, gy + 6);
      ctx.lineTo(gx + Math.sin(t * 0.002 + i) * 2, gy);
      ctx.stroke();
    }
    ...
    drawSheep(
      ctx,
      { x: 0, y: 0, facing: 1, fluff: s.fluff, mood: s.mood, hearts: -999, moving: -999 },
      t,
      game.world.tick,
      true,
    );
    ...
    if (n) {
      ...
      ctx.strokeStyle = 'rgba(67,55,42,.2)';
      ctx.lineWidth = 1.5;
      rr(ctx, 288, 34 + bob, 66, 56, 24);
      ctx.fill();
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(281, 95 + bob, 6.5, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(270, 108 + bob, 3.8, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
      ctx.fillStyle = '#000';
    ...
    for (const s2 of slots) {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(s2.x, s2.y, 28, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
