...
function hit(w, wx, wy) {
  ...
  for (const v of w.villagers)
    if (!v.inside && near(v.x, v.y - 0.3, 16)) return { kind: 'villager', o: v };
  if (w.visitors)
  ...
  for (const t of w.trees)
    if (t.state === 'standing' && near(t.x + 0.5, t.y - 0.1, 16)) return { kind: 'tree', o: t };
  for (const p of w.plots)
    if (tx >= p.x && tx < p.x + 2 && ty >= p.y && ty < p.y + 2) return { kind: 'plot', o: p };
  for (const b of w.buildings) {
    const bw = b.type === 'boat' ? b.w + 2 : b.w; // the boat lies off the end of the landing
    if (tx >= b.x && tx < b.x + bw && ty >= b.y - 1 && ty < b.y + b.h)
      return { kind: 'building', o: b };
  }
  const s = w.bridge.site;
  if (ty >= s.row - 1 && ty <= s.row + s.rows && tx >= s.x0 - 1 && tx <= s.x1 + 1)
    return { kind: 'crossing', o: s };
  ...
  switch (h.kind) {
    case 'tree': {
      ...
      if (tree.state === 'sapling')
        return { title: tr('w.sapling'), hint: tr('w.saplingHint'), actions: [] };
      if (tree.state !== 'standing') {
        const mine = can(w, r, 'farm');
        if (mine)
          A.push({
            label: tr('w.plantHere'),
            fn: () => {
              if (game.dispatch({ type: 'tree.plant', role: r, treeId: tree.id }))
                message(tr('msg.planted'));
            },
          });
        return {
          title: tr('w.stump'),
          hint: tr('w.stumpHint') + (mine ? '' : theirs(game, ['plant'])),
          actions: A,
        };
      }
      ...
      return {
        title: tr('w.tree'),
        hint: tr('w.treeHint') + (canFell ? '' : theirs(game, ['fell'])),
        actions: A,
      };
    }
    ...
    case 'log':
      return {
        title: tr('w.log'),
        hint: tr('w.logHint', { n: h.o.wood }),
        actions: [
          {
            label: tr('w.logTake'),
            fn: () => game.dispatch({ type: 'log.collect', role: r, id: h.o.id }),
          },
        ],
      };
    ...
    case 'stones':
      return {
        title: tr('w.stones'),
        hint: tr('w.stonesHint', { n: h.o.count }),
        actions:
          h.o.count > 0
            ? [
                {
                  label: tr('w.stoneTake'),
                  fn: () => game.dispatch({ type: 'stone.take', role: r, id: h.o.id }),
                },
              ]
            : [],
      };
    ...
    case 'sheep': {
      ...
      if (can(w, r, 'herd'))
        A.push({ label: tr('w.herd'), cls: 'soft', fn: () => game.setMode(sheepMode(game, s)) });
    ...
    case 'plot': {
      ...
      if (!can(w, r, 'farm'))
        return { title: tr('w.plot'), hint: hint + theirs(game, ['farm']), actions: [] };
      if (p.state === 'empty')
        A.push({
          label: tr('w.sow'),
          fn: () => game.dispatch({ type: 'plot.plant', role: r, plotId: p.id, watered: false }),
        });
      if (p.state === 'ripe')
        A.push({
          label: tr('w.reap'),
          fn: () => game.dispatch({ type: 'plot.harvest', role: r, plotId: p.id }),
        });
      if (p.state !== 'empty') {
        A.push({
          label: tr('w.water'),
          cls: p.water <= 8 ? '' : 'soft',
          fn: () => game.dispatch({ type: 'plot.water', role: r, plotId: p.id }),
        });
        const dry = w.plots.filter(q => q.state !== 'empty' && q.water <= 30);
        if (dry.length > 1)
          A.push({
            label: tr('w.waterAll', { n: dry.length }),
            cls: 'soft',
            fn: () => {
              for (const q of dry) game.dispatch({ type: 'plot.water', role: r, plotId: q.id });
            },
          });
      }
    ...
    case 'building': {
      ...
      if (b.type === 'boat') {
        ...
        return {
          title: tr('w.boat'),
          hint: boatHint + (canFish ? '' : theirs(game, ['fish'])),
          actions: A,
        };
      }
      if (b.type === 'play') {
        ...
        const names = kids(w)
          ...
          .join(tr('w.and'));
        return {
          title: tr('w.playground'),
          hint: tr('w.playgroundHint', { names: names }),
          actions: [],
        };
      }
      ...
      if (b.state === 'site') {
        ...
        return {
          title: tr('w.site'),
          hint: siteHint + (mine ? '' : theirs(game, ['house'])),
          actions: A,
        };
      }
      if (b.type === 'workshop') {
        ...
        if (can(w, r, 'mill'))
          A.push({ label: tr('w.millHere'), cls: 'soft', fn: () => openMill(game) });
      ...
      const who = (b.residents || [])
        .map(id => (w.villagers.find(v => v.id === id) || {}).name)
        .filter(Boolean);
      const spare = (b.beds || 0) - (b.residents || []).length;
      return {
        title: tr('w.house'),
        hint:
          (who.length ? trn('w.livesHere', who.length, { names: who.join(tr('w.and')) }) : '') +
    ...
    case 'crossing': {
      ...
      return {
        title: tr('w.crossing'),
        hint:
          tr('w.crossingHint', { n: w.bridge.site.span }) +
          (canBridge ? '' : theirs(game, ['bridge'])),
        actions: A,
      };
    }
    ...
    default: {
      ...
      if (canRoad && stone > 0)
        A.push({ label: tr('w.buildRoad'), fn: () => game.setMode(roadMode(game)) });
      ...
      return {
        title: tr('w.ground'),
        hint: hint + (canRoad ? '' : theirs(game, ['road'])),
        actions: A,
      };
    }
  }
...
export function renderModeBar(game) {
  ...
  if (!modeBar) {
    ...
    modeBar.style.cssText =
      'position:absolute;left:8px;right:8px;bottom:8px;background:#fffdf8;' +
...
export function installInput(game, renderer, canvas) {
  ...
  canvas.addEventListener(
    'wheel',
    e => {
      renderer.cam.zoom = Math.max(
        1,
        Math.min(renderer.maxZoom(), renderer.cam.zoom * (e.deltaY < 0 ? 1.12 : 0.89)),
      );
      renderer.userZoom = true;
      renderer.clampCamera();
      e.preventDefault();
  ...
  canvas.addEventListener('touchend', e => {
    ...
    if (moved > 12 || Date.now() - startT > 700) {
      lastTap = 0;
      return;
    }
    const now = Date.now();
