...
class Bus {
  ...
  join(deliver) { this.peers.push(deliver); return this.peers.length - 1; }
  send(fromId, msg) { for (let i = 0; i < this.peers.length; i++) if (i !== fromId) this.peers[i](msg); }
}

/** Same shape as WsTransport, minus the actual socket. */
class FakeTransport {
  constructor(bus) { this.bus = bus; this.onStatus = null; this.held = null; }
  connect(onMessage) {
    this.onMessage = onMessage;
    this.id = this.bus.join((msg) => {
      if (this.held) this.held.push(msg); else this.onMessage(msg);
    });
    ...
    Promise.resolve().then(() => { if (this.onStatus) this.onStatus('online'); });
    return Promise.resolve();
  }
  /** Queue what arrives instead of delivering it, to force a race on purpose. */
  hold() { this.held = []; }
  release() { const h = this.held || []; this.held = null; for (const m of h) this.onMessage(m); }
  send(msg) { this.bus.send(this.id, msg); }
  close() {}
}
...
async function paired(room) {
  const bus = new Bus();
  const ta = new FakeTransport(bus), tb = new FakeTransport(bus);
...
test('a reconnect says hello again, so the other side knows to catch us up', async () => {
  ...
  assert.ok(heard.some(m => m.t === 'hello'), 'a reconnect said hello again');
});
