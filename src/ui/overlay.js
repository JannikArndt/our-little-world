...
export function openPanel(opts) {
  ...
  const costSlot = el('div', 'p-cost'),
    roSlot = el('div', 'p-readout'),
    rowSlot = el('div', 'p-rows');
  foot.appendChild(costSlot);
  foot.appendChild(roSlot);
  foot.appendChild(rowSlot);
  ...
  const api = {
    ...
    readout(text) {
      if (!api._ro) {
        api._ro = el('div', 'readout');
        roSlot.appendChild(api._ro);
      }
    ...
    cost(items) {
      if (!api._cost) {
        ...
        costSlot.appendChild(api._cost);
      }
...
export function renderCost(host, items) {
  ...
  for (const it of items) {
    ...
    if (!enough) {
      ...
      short.appendChild(
        document.createTextNode(' ' + it.icon + ' ' + trn('ui.stillMissing', it.need - it.have)),
      );
...
export function makeCanvas(w, h) {
  ...
  const fit = () => {
    ...
    const bw = Math.round(rect.width * dpr),
      bh = Math.round(rect.width * (h / w) * dpr);
    if (c.width !== bw) {
      c.width = bw;
      c.height = bh;
    }
...
export function onPointer(canvas, logicalW, logicalH, handlers) {
  ...
  const start = e => {
    e.preventDefault();
    down = true;
    if (handlers.down) handlers.down(pos(e));
  };
  const move = e => {
    if (!down) {
      if (handlers.hover) handlers.hover(pos(e));
      return;
    }
    e.preventDefault();
    if (handlers.move) handlers.move(pos(e));
  };
  const end = e => {
    if (!down) return;
    down = false;
    if (handlers.up) handlers.up(pos(e));
  };
...
export function openMenu(anchor, opts) {
  ...
  for (const it of opts.items || []) {
    ...
    const b = el(
      'button',
      'menu-item' + (it.disabled ? ' off' : '') + (it.on ? ' on' : '') + (it.sub ? ' sub' : ''),
    );
