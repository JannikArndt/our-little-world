 ...
 */
export const SHAPES = {
  bed:     [[0.06, 0.34, 0.06, 0.78, 0.94, 0.78, 0.94, 0.46, 0.34, 0.46, 0.34, 0.34, 0.06, 0.34],
            [0.1, 0.78, 0.1, 0.92], [0.9, 0.78, 0.9, 0.92]],
  window:  [[0.16, 0.12, 0.84, 0.12, 0.84, 0.88, 0.16, 0.88, 0.16, 0.12],
            [0.5, 0.12, 0.5, 0.88], [0.16, 0.5, 0.84, 0.5]],
  lamp:    [[0.38, 0.34, 0.38, 0.9, 0.62, 0.9, 0.62, 0.34, 0.38, 0.34],
            [0.5, 0.34, 0.38, 0.2, 0.5, 0.05, 0.62, 0.2, 0.5, 0.34]],
  shelf:   [[0.08, 0.6, 0.92, 0.6], [0.24, 0.6, 0.28, 0.4, 0.44, 0.3, 0.62, 0.34, 0.7, 0.48, 0.68, 0.6]],
  stove:   [[0.5, 0.06, 0.28, 0.34, 0.22, 0.6, 0.34, 0.84, 0.5, 0.94, 0.7, 0.86, 0.8, 0.62, 0.7, 0.34, 0.5, 0.06],
            [0.5, 0.46, 0.4, 0.66, 0.46, 0.84, 0.58, 0.78, 0.58, 0.6, 0.5, 0.46]],
  table:   [[0.06, 0.44, 0.94, 0.44], [0.2, 0.44, 0.24, 0.9], [0.8, 0.44, 0.76, 0.9],
            [0.34, 0.28, 0.5, 0.22, 0.66, 0.28, 0.62, 0.4, 0.38, 0.4, 0.34, 0.28]],
  chair:   [[0.3, 0.12, 0.3, 0.56, 0.72, 0.56], [0.3, 0.56, 0.26, 0.92], [0.72, 0.56, 0.74, 0.92]],
  blanket: [[0.1, 0.3, 0.9, 0.3, 0.9, 0.76, 0.1, 0.76, 0.1, 0.3],
            [0.1, 0.53, 0.3, 0.45, 0.5, 0.6, 0.7, 0.45, 0.9, 0.53]],
  flowers: [[0.5, 0.5, 0.5, 0.94], [0.5, 0.5, 0.3, 0.62],
            [0.28, 0.3, 0.34, 0.5, 0.5, 0.56, 0.66, 0.5, 0.72, 0.3, 0.58, 0.42, 0.5, 0.2, 0.42, 0.42, 0.28, 0.3]],
};
...
export function wordStrokes(word, x, y, w, h) {
  const letters = String(word).toUpperCase().split('').filter((c) => LETTERS[c]);
  ...
  for (let i = 0; i < letters.length; i++) {
    ...
    for (const s of place(LETTERS[letters[i]], lx, y, cell, h)) { s.group = i; out.push(s); }
  }
  return out;
}
...
export function tracer(strokes, span) {
  ...
  function offer(p) {
    for (const s of strokes) {
      ...
      for (let k = s.at; k < Math.min(s.pts.length, s.at + 3); k++) {
        const q = s.pts[k];
        if (Math.hypot(p.x - q.x, p.y - q.y) <= reach) { s.at = k + 1; break; }
      }
    }
  }
  ...
  return {
    strokes: strokes,
    down(p) { drawing = true; trail = [p]; offer(p); },
    move(p) {
      if (!drawing) return;
    ...
    up() { drawing = false; trail = []; },
    /** How far along, 0 to 1 — for anything that wants to show progress. */
    howFar() {
      let did = 0, all = 0;
      for (const s of strokes) { did += Math.min(s.at, s.pts.length); all += s.pts.length; }
      return all ? did / all : 1;
    },
    done() { return strokes.every((s) => s.at >= s.pts.length); },
    draw(ctx, t) {
...
export function traceHeader(ctx, icon, label, x, y, w) {
  ...
  ctx.strokeStyle = 'rgba(217,201,174,.9)'; ctx.lineWidth = 2;
  rr(ctx, x, y, w, 34, 12); ctx.fill(); ctx.stroke();
